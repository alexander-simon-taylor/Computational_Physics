...
data1 = np.loadtxt('Side_view_data_run1.txt')
t1 = data1[:,0]
radius1 = data1[:,1]*1e-6
# t1 is the time data, it is identical for each data set in the same run, so is only defined once.

data2 = np.loadtxt('Side_view_data_run2.txt')
radius2 = data2[:,1]*1e-6

data3 = np.loadtxt('Side_view_data_run3.txt')
radius3 = data3[:,1]*1e-6

data4 = np.loadtxt('Side_view_data_run4.txt')
radius4 = data4[:,1]*1e-6

data5 = np.loadtxt('Side_view_data_run5.txt')
radius5 = data5[:,1]*1e-6

data6 = np.loadtxt('Side_view_data_run6.txt')
radius6 = data6[:,1]*1e-6

AngleSpeed1 = AngleSpeed(radius1)
AngleSpeed2 = AngleSpeed(radius2)
AngleSpeed3 = AngleSpeed(radius3)
AngleSpeed4 = AngleSpeed(radius4)
AngleSpeed5 = AngleSpeed(radius5)
AngleSpeed6 = AngleSpeed(radius6)
...
velocity[3] = AngleSpeed4[1]
velocity[4] = AngleSpeed5[1]
velocity[5] = AngleSpeed6[1]

PlotData = (AngleSpeed1 + AngleSpeed2 + AngleSpeed3 + AngleSpeed4 + AngleSpeed5 + AngleSpeed6)/6
...
for i in range(0,len(t1)-2):
    ...
    sigmaMes[i] = math.sqrt(iSigma/5)/math.sqrt(5)
...
FittedAxes = PolynomialFit(PlotData[0],PlotData[1],PlotData[2],3)
...
plt.title("Plot of the side view data fitted to a general cubic with propogated errors.")

plt.ylabel("Speed of contact point (μm/s)")

plt.plot(PlotData[0],PlotData[1],"b.") # Plots the v data against theta
plt.errorbar(PlotData[0],PlotData[1],yerr=PlotData[2],fmt="none") # Puts in the error bars
...
#print("The reduced Chi Squared value is %f" % (deGennesAxes[2])) # Cox Voinov polynomial prinout.
print(deGennesAxes[3])