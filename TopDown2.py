...
PlotData = (AngleSpeed1 + AngleSpeed2)/2;
...
FittedAxes = PolynomialFit(PlotData[0],PlotData[1],sigmaMes,1)
...
eqAngle = 90
for j in range(0,1): # 2 or 3 depending on cubic or quadratic fit
    root = allAngle[j]
    if abs(root.imag) < 1e-5: #Finding real roots, since numerical methods could give a tiny imaginary part.
...
plt.title("Plot of the top view large contact angle data fitted to a Cox-Voinov spreading law.")
